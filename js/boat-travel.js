 ...
 */
export function startBoatVoyage({ from, to, camera, overlayEl, duration = 2.8, onMidpoint, onComplete }) {
  return new Promise((resolve) => {
    ...
    const tick = () => {
      ...
      if (p < 0.5) {
        camera.position.lerpVectors(start, mid, ease * 2);
      } else {
        if (p >= 0.5 && onMidpoint) onMidpoint();
        const local = (p - 0.5) * 2;
        camera.position.lerpVectors(mid, end, local);
      }
